...
function showPlaceholder(){
  document.getElementById('rules-display').innerHTML = '<p class="muted">As regras serão adicionadas aqui em breve...</p>';
}

function openEditor(){
  const current = localStorage.getItem(RULES_KEY) || '';
  document.getElementById('rules-textarea').value = current;
...
function saveRules(){
  ...
  if (!text){
    if (!confirm('O texto está vazio. Deseja salvar mesmo assim?')) return;
  }
  localStorage.setItem(RULES_KEY, text);
  closeEditor();
  if (text) showRules(text);
  else showPlaceholder();
...
function copyRules(){
  ...
  if (!text){
    alert('Não há regras salvas para copiar.');
    return;
  }
  if (navigator.clipboard && navigator.clipboard.writeText){
    navigator.clipboard.writeText(text).then(()=> alert('Regras copiadas!'))
...
function editName(){
  ...
  if (newName !== null){
    localStorage.setItem(NAME_KEY, newName.trim() || 'Regras do Grupo');
    document.getElementById('group-name').textContent = localStorage.getItem(NAME_KEY);
  }
}
